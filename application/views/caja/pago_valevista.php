...
<form method="post" action="/caja/cantidaddoctos/3861/<?=$idpostulacionitem?>">
...
<tr>
<td width="135" ><strong>Cant. vale vistas</strong></td>
<td><select name="cant" onchange="submit();">
<optgroup label="Cantidad de transferencias"><?
foreach($this->config->item('total_valevista') as $value){
	?>
...
<form method="post" id="form_validate" action="/caja/registrarpago/3861/<?=$idpostulacionitem?>">
<input type="hidden" name="cant" value="<?=$rango?>" />
<?
foreach(range(1,$rango) as $key => $value){		
	?>
    <fieldset>
    <legend><strong>VALE VISTA NRO. <?=($key+1)?></strong></legend> 
    <table width="100%" border="0" cellpadding="0" cellspacing="0" align="center" >      
    <tr>
    ...
    ?>
    <option value="<?=$value['IDMetadatoDetalle']?>" <?=(($pagados[$key]['IDBanco'] == $value['IDMetadatoDetalle']) AND (count($pagados) > 0))?'selected="selected"':'';?> ><?=$value['dscrn']?></option><?
    }?>    
    </optgroup>
    </select></td>
    </tr>
    ...
    <td><input type="text" name="fecha[<?=$key?>]" id="calendar<?=$key?>" value="<?=(!$pagados[$key]['FechaValeVista'])?date('d/m/Y'):$pagados[$key]['FechaValeVista']?>" class="center calendar" size="10" readonly="readonly"  /></td>
    </tr>
	<tr>
	<td><strong>Nro. Vale Vista</strong></td>
    <td><input type="text" name="comprobante[<?=$key?>]" value="<?=$pagados[$key]['NumeroValeVista']?>" style="width:97.5%;" class="obl ctacte" /></td>
	</tr>
	<tr>
	  <td><strong>Monto</strong></td>
	  <td><input type="text" name="monto[]" value="<?=number_format($pagados[$key]['Monto'],0,',','.')?>" style="width:97.5%;" class="obl right" maxlength="10" /> 
      <input type="hidden"  name="id_pago[<?=$key?>]" value="<?=$pagados[$key]['IDPagoValeVista']?>"/></td>
    </tr>
	</table>
	</fieldset><?	
}?>

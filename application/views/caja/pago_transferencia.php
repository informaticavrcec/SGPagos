...
<form method="post" action="/caja/cantidaddoctos/3862/<?=$idpostulacionitem?>">
...
<tr>
<td width="135" ><strong>Cant. transferencias</strong></td>
<td><select name="cant" onchange="submit();">
<optgroup label="Cantidad de transferencias"><?
foreach($this->config->item('total_transferencia') as $value){
	?>
...
<form method="post" id="form_validate" action="/caja/registrarpago/3862/<?=$idpostulacionitem?>">
<input type="hidden" name="cant" value="<?=$rango?>" /><?
foreach(range(1,$rango) as $key => $value){		
	?>
    <fieldset>
    <legend><strong>TRANSFERENCIA NRO. <?=($key+1)?></strong></legend> 
    <table width="100%" border="0" cellpadding="0" cellspacing="0" align="center" >        
    <tr>
	...
	<tr>
	<td><strong>Nro. transferencia</strong></td>
    <td><input type="text" name="comprobante[<?=$key?>]" value="<?=$pagados[$key]['NumeroTransferencia']?>" style="width:97.5%;" class="obl ctacte" /></td>
	</tr>
	<tr>
	  <td><strong>Monto</strong></td>
	  <td><input type="text" name="monto[]" value="<?=number_format($pagados[$key]['Monto'],0,',','.')?>" style="width:97.5%;" class="obl right" maxlength="10" /> 
      <input type="hidden"  name="id_pago[<?=$key?>]" value="<?=$pagados[$key]['IDPagoTransferencia']?>"/></td>
    </tr>
	</table>
	</fieldset><?	
}?>
